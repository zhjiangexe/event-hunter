...
func TestSaveReevaluatesAndPersistsImmutableEvidenceReferences(t *testing.T) {
	...
	evaluator := evaluateeventcheck.NewService(source)
	evaluationRequest := evaluateeventcheck.Request{
		Identifier: evaluateeventcheck.Identifier{Type: "CORRELATION_ID", Value: "ORDER-SNAPSHOT-01"},
	...
	service := NewService(evaluator, repository, repository, directUnitOfWork{})
	service.now = func() time.Time { return start.Add(11 * time.Minute) }
	created, wasCreated, err := service.Save(context.Background(), Request{
...
func TestSaveRejectsLateEventInsteadOfSilentlyChangingSnapshot(t *testing.T) {
	...
	source := &mutableSource{result: ports.CanonicalEventResult{Events: deviatedEvents(start)}}
	evaluator := evaluateeventcheck.NewService(source)
	request := evaluateeventcheck.Request{
		Identifier: evaluateeventcheck.Identifier{Type: "CORRELATION_ID", Value: "ORDER-SNAPSHOT-01"},
		From:       start.Format(time.RFC3339), To: start.Add(10 * time.Minute).Format(time.RFC3339),
	}
	...
	repository := newMemorySnapshotRepository()
	service := NewService(evaluator, repository, repository, directUnitOfWork{})
	_, _, err = service.Save(context.Background(), Request{
...
func TestSaveIsIdempotentAndRejectsKeyReuse(t *testing.T) {
	...
	source := &mutableSource{result: ports.CanonicalEventResult{Events: deviatedEvents(start)}}
	evaluator := evaluateeventcheck.NewService(source)
	evaluation, err := evaluator.Evaluate(context.Background(), evaluateeventcheck.Request{
		Identifier: evaluateeventcheck.Identifier{Type: "CORRELATION_ID", Value: "ORDER-SNAPSHOT-01"},
		From:       start.Format(time.RFC3339), To: start.Add(10 * time.Minute).Format(time.RFC3339),
	})
	if err != nil {
	...
	repository := newMemorySnapshotRepository()
	service := NewService(evaluator, repository, repository, directUnitOfWork{})
	input := Request{EvaluationRequest: evaluation.NormalizedRequest, ExpectedEventSetHash: *evaluation.EventSetHash, ExpectedEvaluationHash: *evaluation.EvaluationHash}
