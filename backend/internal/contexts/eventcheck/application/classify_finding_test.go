...
type feedbackUnitOfWork struct{}

func (feedbackUnitOfWork) WithinTransaction(ctx context.Context, operation func(context.Context) error) error {
	return operation(ctx)
}

func TestClassifyCreatesAndOptimisticallyUpdatesFeedback(t *testing.T) {
	repository := &feedbackRepository{}
	service := NewClassifyFindingHandler(repository, repository, feedbackUnitOfWork{})
