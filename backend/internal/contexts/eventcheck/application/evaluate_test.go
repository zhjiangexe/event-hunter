...
func TestEvaluateResolvesScopeAndProducesStableHashes(t *testing.T) {
	...
	service := NewEvaluateEventCheckHandler(fakeEventSource{result: ports.CanonicalEventResult{Events: events, Watermark: &watermark}})
...
func TestEvaluateNoDataIsAProductStateWithoutHashes(t *testing.T) {
	from := time.Date(2026, 8, 28, 0, 0, 0, 0, time.UTC)
	service := NewEvaluateEventCheckHandler(fakeEventSource{result: ports.CanonicalEventResult{Events: []domain.Event{}}})
...
func TestEvaluateCustomExclusionPreservesReasonAndChangesOutcome(t *testing.T) {
	from := time.Date(2026, 8, 28, 0, 0, 0, 0, time.UTC)
	service := NewEvaluateEventCheckHandler(fakeEventSource{result: ports.CanonicalEventResult{Events: fulfillmentEvents(from)}})
	result, err := service.Evaluate(context.Background(), EvaluateRequest{
		Identifier: Identifier{Type: "CORRELATION_ID", Value: "ORDER-CHECK-01"},
		From:       from.Format(time.RFC3339), To: from.Add(10 * time.Minute).Format(time.RFC3339),
