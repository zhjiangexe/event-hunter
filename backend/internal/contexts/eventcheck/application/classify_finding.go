...
type ClassifyFindingHandler struct {
...
func NewClassifyFindingHandler(repository ports.SnapshotRepository, audit ports.AuditWriter, unitOfWork ports.UnitOfWork) *ClassifyFindingHandler {
	return &ClassifyFindingHandler{repository: repository, audit: audit, unitOfWork: unitOfWork, now: time.Now}
}

func (service *ClassifyFindingHandler) Classify(ctx context.Context, findingID string, status domain.FindingFeedbackStatus, expectedVersion int64, actor domain.SnapshotActor, requestID string) (domain.FindingFeedback, error) {
	now := service.now().UTC()
