...
func FromDomain(snapshot domain.CheckSnapshot) (Snapshot, error) {
	...
	response := Snapshot{
		...
		Result: result, EventReferences: []EventReference{}, Relationships: []evaluateeventcheck.Relationship{}, FindingFeedback: defaultFindingFeedback(result),
	...
	for _, reference := range snapshot.EventReferences {
		response.EventReferences = append(response.EventReferences, EventReference{
