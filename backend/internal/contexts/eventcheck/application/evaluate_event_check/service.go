...
func NewService(source ports.CanonicalEventSource) *Service {
	return &Service{source: source, registry: domain.ActiveRegistry(), now: time.Now}
}

func (service *Service) Evaluate(ctx context.Context, input Request) (Response, error) {
	...
	if err != nil {
		return Response{}, fmt.Errorf("find canonical events: %w", err)
	}
	...
	if err != nil {
		return Response{}, fmt.Errorf("hash event scope: %w", err)
	}
	evaluationHash, err := EvaluationHash(request, model, eventSetHash, health, result)
	if err != nil {
		return Response{}, fmt.Errorf("hash evaluation: %w", err)
	}
...
func (service *Service) selectModel(requested *RequestedModel, candidates []domain.ModelCandidate) (domain.RegistryEntry, domain.ResolutionStatus, error) {
	if requested != nil {
...
func normalizeRequest(input Request) (Request, time.Time, time.Time, error) {
	...
	if !validIdentifier[input.Identifier.Type] || input.Identifier.Value == "" || len(input.Identifier.Value) > 200 {
		return Request{}, time.Time{}, time.Time{}, fmt.Errorf("%w: identifier", ErrInvalidRequest)
	}
	from, err := time.Parse(time.RFC3339Nano, strings.TrimSpace(input.From))
	if err != nil {
		return Request{}, time.Time{}, time.Time{}, fmt.Errorf("%w: from", ErrInvalidRequest)
	}
	...
	if err != nil || !from.Before(to) || to.Sub(from) > domain.PlatformMaxDuration {
		return Request{}, time.Time{}, time.Time{}, fmt.Errorf("%w: time window", ErrInvalidRequest)
	}
...
func (service *Service) sourceHealth(from, to time.Time, source ports.CanonicalEventResult) SourceHealth {
...
func EvaluationHash(request Request, model ModelRef, eventSetHash string, health SourceHealth, result CheckResult) (string, error) {
	return canonicaljson.SHA256(map[string]any{
