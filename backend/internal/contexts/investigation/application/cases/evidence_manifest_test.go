...
func TestGenerateBuildsVerifiablePartialManifest(t *testing.T) {
	...
	service := NewEvidenceManifestService(manifestReaderFake{
	...
	result, err := service.Generate(t.Context(), EvidenceManifestRequest{InvestigationID: "case-1", From: &from, To: &now})
	if err != nil {
...
func TestGenerateRestrictsGrafanaAlertOpenAction(t *testing.T) {
	...
	result, err := service.Generate(t.Context(), EvidenceManifestRequest{InvestigationID: "case-1"})
	if err != nil {
		t.Fatalf("Generate() error = %v", err)
	}
