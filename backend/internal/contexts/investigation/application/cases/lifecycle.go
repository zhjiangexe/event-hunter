...
func NewCaseService(cases CaseRepository, details InvestigationDetailsRepository, unitOfWork ports.UnitOfWork) *CaseService {
	if unitOfWork == nil {
		panic("case lifecycle requires a UnitOfWork")
	}
	return &CaseService{cases: cases, details: details, unitOfWork: unitOfWork, now: time.Now}
}

func (service *CaseService) List(ctx context.Context, filter CaseFilter) (CasePage, error) {
	return service.cases.List(ctx, filter)
}

func (service *CaseService) Create(ctx context.Context, title string, severity domain.Severity, correlationID string, incidentWindow domain.IncidentWindow, actor Actor, requestID string) (domain.InvestigationCase, error) {
	now := service.now().UTC()
...
func (service *CaseService) Get(ctx context.Context, id string) (domain.InvestigationCase, error) {
	return service.cases.Get(ctx, id)
}

func (service *CaseService) GetDetails(ctx context.Context, id string) (CaseDetails, error) {
...
func (service *CaseService) GetSummaryDetails(ctx context.Context, id string) (SummaryDetails, error) {
	...
	if err != nil {
		return SummaryDetails{}, fmt.Errorf("load investigation audit: %w", err)
	}
	return SummaryDetails{CaseDetails: details, Audit: audit}, nil
}

func (service *CaseService) Update(ctx context.Context, id string, expectedVersion int64, patch CasePatch, actor Actor, requestID string) (domain.InvestigationCase, error) {
...
func (service *CaseService) Close(ctx context.Context, id string, expectedVersion int64, rootCause, resolutionSummary string, fixedVersion *string, actor Actor, requestID string) (domain.InvestigationCase, error) {
	var updated domain.InvestigationCase
	err := service.withinTransaction(ctx, func(transactionContext context.Context) error {
		current, err := service.cases.Get(transactionContext, id)
		if err != nil {
...
func (service *CaseService) Audit(ctx context.Context, id string) ([]AuditEntry, error) {
	return service.details.Audit(ctx, id)
}

func (service *CaseService) Findings(ctx context.Context, id string) ([]PatternFinding, error) {
	return service.details.Findings(ctx, id)
}

func (service *CaseService) Evidence(ctx context.Context, id string) ([]Evidence, error) {
	return service.details.Evidence(ctx, id)
}

func (service *CaseService) Notes(ctx context.Context, id string) ([]domain.CaseNote, error) {
	return service.details.Notes(ctx, id)
}

func (service *CaseService) AddNote(ctx context.Context, id string, expectedVersion int64, body string, actor Actor, requestID string) (domain.InvestigationCase, domain.CaseNote, error) {
	var updated domain.InvestigationCase
...
func (service *CaseService) SaveFinding(ctx context.Context, id string, finding PatternFinding) error {
	return service.details.SaveFinding(ctx, id, finding)
}

func (service *CaseService) SaveEvidence(ctx context.Context, id, evidenceType, reference, checksum string) error {
	return service.details.SaveEvidence(ctx, id, evidenceType, reference, checksum)
}

func (service *CaseService) RecordAudit(ctx context.Context, actor Actor, action, resourceID, requestID string, metadata map[string]any) error {
	return service.details.RecordAudit(ctx, actor, action, resourceID, requestID, metadata)
}

func (service *CaseService) withinTransaction(ctx context.Context, operation func(context.Context) error) error {
