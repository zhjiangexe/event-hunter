...
func TestGetReturnsCaseAndEventSourceResult(t *testing.T) {
	...
	service := NewSummaryService(summaryCaseReaderFake{details: SummaryDetails{CaseDetails: CaseDetails{Case: summaryCase("case-1", "ORDER-1")}}}, reader)
	service.now = func() time.Time { return generatedAt }

	from := generatedAt.Add(-time.Hour)
	result, err := service.Get(t.Context(), SummaryRequest{InvestigationID: "case-1", From: &from, To: &generatedAt, Limit: 1000})
	if err != nil {
...
func TestGetKeepsPostgresDetailsWhenEventSourceTimesOut(t *testing.T) {
	service := NewSummaryService(
		summaryCaseReaderFake{details: SummaryDetails{CaseDetails: CaseDetails{Case: summaryCase("case-1", "ORDER-1")}}},
		&summaryEventReaderFake{searchErr: context.DeadlineExceeded},
	)
	result, err := service.Get(t.Context(), SummaryRequest{InvestigationID: "case-1", Limit: 1000})
	if err != nil {
		t.Fatalf("Get() error = %v", err)
	}
...
func TestGetReturnsRequiredCaseReadFailure(t *testing.T) {
	...
	service := NewSummaryService(summaryCaseReaderFake{err: want}, &summaryEventReaderFake{})
	if _, err := service.Get(t.Context(), SummaryRequest{}); !errors.Is(err, want) {
...
func (fake summaryCaseReaderFake) GetSummaryDetails(context.Context, string) (SummaryDetails, error) {
