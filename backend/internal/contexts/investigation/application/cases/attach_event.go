...
type EventEvidenceService struct {
...
func NewEventEvidenceService(cases domain.CaseEvidenceRepository, events EventLookup, audit AuditWriter, unitOfWork ports.UnitOfWork) *EventEvidenceService {
	if unitOfWork == nil {
		panic("evidence attachment requires a UnitOfWork")
	}
	return &EventEvidenceService{cases: cases, events: events, audit: audit, unitOfWork: unitOfWork, now: time.Now}
}

func (service *EventEvidenceService) AttachEvent(ctx context.Context, command AttachEventCommand) (AttachEventResult, error) {
...
func (service *EventEvidenceService) withinTransaction(ctx context.Context, operation func(context.Context) error) error {
