...
func TestUpdateAppliesStateTransitionAndRecordsAudit(t *testing.T) {
	...
	service := NewCaseService(repository, details, directUnitOfWork{})
...
func TestUpdateRejectsInvalidAndIncompleteResolvedTransitions(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			service := NewCaseService(repository, &detailsRepositoryFake{}, directUnitOfWork{})
...
func TestGetDetailsAndSummaryFailWhenRequiredChildReadFails(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			service := NewCaseService(&caseRepositoryFake{current: domain.InvestigationCase{ID: "case-1"}}, testCase.details, directUnitOfWork{})
			var err error
			if testCase.summary {
...
func TestCaseMutationsRollBackWhenAuditFails(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			service := NewCaseService(repository, details, unit)
