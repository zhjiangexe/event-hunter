...
func (service *Service) Generate(ctx context.Context, request Request) (Result, error) {
	...
	from, to, err := investigationwindow.Resolve(request.From, request.To, loadedCase.IncidentWindow)
	if err != nil {
		return Result{}, err
	}
	...
	if err != nil {
		return Result{}, fmt.Errorf("encode evidence manifest: %w", err)
	}
...
func (result Result) Manifest() map[string]any { return result.manifestMap(true) }

func (result Result) manifestMap(includeDigest bool) map[string]any {
...
func (item Item) Response() map[string]any { return item.manifestMap() }

func (item Item) manifestMap() map[string]any {
...
func ItemsFromEvidence(evidence []ports.Evidence) []Item {
...
func itemFromEvidence(evidence ports.Evidence) Item {
