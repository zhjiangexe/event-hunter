...
type PatternEffectivenessService struct {
	reader Reader
	now    func() time.Time
}

func NewPatternEffectivenessService(reader Reader) *PatternEffectivenessService {
	return &PatternEffectivenessService{reader: reader, now: time.Now}
}

func (service *PatternEffectivenessService) Get(ctx context.Context) (Summary, error) {
