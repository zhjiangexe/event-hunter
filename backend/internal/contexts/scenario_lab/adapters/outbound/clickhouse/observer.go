...
func (observer *Observer) Observe(ctx context.Context, correlationID string) (domain.Actual, error) {
	actual := domain.EmptyActual()
	...
	for _, event := range events {
		...
		occurred, oe := parseTime(event.OccurredAt)
		ingested, ie := parseTime(event.IngestedAt)
		if oe == nil && ie == nil && ingested.After(occurred) {
	...
	actual.ProcessingStatuses, err = observer.stringColumn(ctx, `SELECT processing_status AS value FROM canonical_event_processing_attempts WHERE correlation_id={correlation:String} ORDER BY observed_at,attempt_id FORMAT JSON`, correlationID)
	if err != nil {
		return actual, err
	}
	actual.IngestionFailureTypes, err = observer.stringColumn(ctx, `SELECT value FROM (SELECT error_type AS value,failed_at AS occurred_at,source_offset FROM event_ingestion_failures WHERE correlation_id={correlation:String} UNION ALL SELECT error_code AS value,failed_at AS occurred_at,source_offset FROM poc_event_admission_failures WHERE correlation_id={correlation:String}) ORDER BY occurred_at,source_offset FORMAT JSON`, correlationID)
	if err != nil {
		return actual, err
	}
...
func (observer *Observer) events(ctx context.Context, correlationID string) ([]observedEvent, error) {
	data, err := observer.query(ctx, `SELECT event_id,event_type,aggregate_type,aggregate_id,trace_id,sequence,occurred_at,ingested_at FROM canonical_forensics_events WHERE correlation_id={correlation:String} ORDER BY occurred_at,event_id FORMAT JSON`, correlationID)
	if err != nil {
		return nil, err
	}
	var result struct {
	...
	for _, item := range result.Data {
		events = append(events, observedEvent{item.EventID, item.EventType, item.AggregateType, item.AggregateID, item.TraceID, item.OccurredAt, item.IngestedAt, item.Sequence})
	}
	return events, nil
}
func (observer *Observer) stringColumn(ctx context.Context, statement, correlationID string) ([]string, error) {
...
func (observer *Observer) query(ctx context.Context, statement, correlationID string) ([]byte, error) {
...
func (observer *Observer) client() *http.Client {
