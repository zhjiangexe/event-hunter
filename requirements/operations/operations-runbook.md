...
內部單機 staging／pilot 若需要 TLS edge、非預設 secret 與關閉所有內部 host ports，必須搭配
[Single-host Hardening Baseline](single-host-hardening-baseline.md) 與 `compose.hardening.yaml`；兩者仍不等於
正式 HA production 架構。
...
第一個會停止 ClickHouse，要求 evaluation 回 `503 EVENT_CHECK_SOURCE_UNAVAILABLE` 且不產生假的業務結果，
恢復後同一 request 的 evaluation hash 必須一致。第二個會建立 Snapshot／Case link，重啟 PostgreSQL 與
Event Hunter API，再逐欄比對 immutable Snapshot 與關聯，最後透過正式 API 結案。兩者都有 EXIT trap
恢復服務，但仍只應在允許短暫中斷的本機／隔離環境執行。
...
| Event Check 回 `INCONCLUSIVE` | `source_health` 是否 PARTIAL、事件是否 truncated、Model warnings | 不可改判為 `VIOLATED`；先恢復缺少的 canonical evidence，再以同一 bounded request 重新執行 |
| Event Check 回 `503 EVENT_CHECK_SOURCE_UNAVAILABLE` | ClickHouse ready、API logs、同一 request 的 bounded window | 先恢復 ClickHouse；來源完全不可讀時沒有可信的業務判定，不應建立假 Finding |
...
`requirements/delivery/archive/phase-1-1-sign-off.md`。
