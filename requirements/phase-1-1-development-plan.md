...
- 狀態：`in_progress`（`2026-08-24` 已完成並簽核的是 **Phase 1.1 本機 release baseline**；
  案件進階排序／複合篩選也已補完，目前只剩正式部署 hardening 尚待目標環境決策。
...
SLA 時窗以 `requirements/case-collaboration-contract.md` 為準。
...
4. 本輪問題都屬既有 `REQ-EH-*` 的正確性與產品化修正，不新增 scope；工程 DAG 以
   `requirements/implementation-plan.yaml` 為準。
...
- 依據：`requirements/traceability.yaml`（新增或綁定 REQ-EH-*）、OpenAPI、service contract、DB query budget
...
- 內容：event detail 提供「加入既有案件」；後端以原始 bounded window 精確驗證 ClickHouse event，
  PostgreSQL 只保存 EVENT reference、SHA-256 與來源 correlation 關聯，不保存 payload
- 契約：`REQ-EH-015`、OpenAPI `attachInvestigationEvent`、
  `requirements/timeline-evidence-attachment-contract.md`
...
- **已完成基準**：Phase 1.1 本機 release baseline、Wave C、P1.1-06-02、P1.1-07-01、
  P1.1-08-01、EH-POC-002 與 Wave E 已驗收；此 sign-off 不代表原始產品化計畫全部完成
- **最近完成**：P1.1-04-01 Pattern 成效度量、P1.1-04-02 Pattern 回饋／Git-based 治理
- **既有核准交付範圍**：本機產品工作已完成；P1.1-02-03 已完成
- **新增 UX remediation 工作**：2026-08-26 逐頁桌面／390px 稽核另發現時間可信度、案件工作流、
  Journey／Pattern／Scenario 判讀與 responsive 缺口；依
  `requirements/phase-1-1-product-ux-remediation-plan.md` 排序。`P1.1-UX-01`～`P1.1-UX-04`／
  `EH-P1.1-013`～`EH-P1.1-016` 已納入正式 implementation DAG 並完成；使用者另行核准的
  `P1.1-UX-13` 接入指南子範圍由 `EH-P1.1-017` 完成。主序列下一項仍為全頁面 390px 核心工作流修復的
  `P1.1-UX-05`，其餘後續工作須逐項核准與驗收
- **待部署決策**：P1.1-08-02 正式部署 Hardening 與 staging 演練
