...
- Investigation application service 已依 screaming architecture 按業務能力拆分為 `case_lifecycle`、`forensics`、`event_search`、`pattern_analysis` 與 `alert_intake`；舊平面 package 與通用 facade 已移除，正式 API composition root 與 infrastructure adapter 直接依賴各能力邊界。詳細邊界見 `requirements/application-screaming-architecture.md`。
