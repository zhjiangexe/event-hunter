...
- 更新日期：2026-08-29
- 適用範圍：目前本機 Compose、Go backend、React frontend、正式 ClickHouse-first ingestion、OpenTelemetry、Phase 1.1 UI 與 Event Check／Check Models workspace
...
驗收見 [Live Event Observability Contract](../contracts/live-event-observability-contract.md)。
...
```mermaid
flowchart TD
    Entry([收到異常或已知識別碼]) --> Locate[Overview / Event Check 輸入 ID]
    Locate --> Scope[解析 bounded scope 與關聯 provenance]
    Scope --> Model{Check Model 候選是否唯一？}
    Model -- 否 --> Select[使用者確認或切換 Model]
    Model -- 是 --> Evaluate[執行 deterministic Event Check]
    Select --> Evaluate
    Evaluate --> Result[Summary / Timeline / Flow / Findings]
    Result --> Technical{需要技術證據？}
    Technical -- 是 --> Grafana[以 deep link 開啟 Grafana / Tempo / Loki]
    Technical -- 否 --> SaveDecision
    Grafana --> SaveDecision{需要保存當下判斷？}
    SaveDecision -- 是 --> Snapshot[保存 immutable Check Snapshot / Evidence refs]
    SaveDecision -- 否 --> CaseDecision
    Snapshot --> CaseDecision{需要協作與追蹤？}
    CaseDecision -- 否 --> EndRead([完成唯讀調查])
    CaseDecision -- 是 --> Case[建立或開啟 Investigation Case]
    Case --> Work[掛入 Snapshot、指派、Notes、Finding feedback、Audit]
...
Event Check 是 query-time、唯讀的流程符合性檢查，不是 production workflow state machine。Flow 與
Expectations 來自使用者確認的 immutable Check Model；前端不自行推導結果。Business Journey、Journey
Profiles 與 Pattern Library 的 bounded 舊網址保留為 compatibility adapter，不再是主要產品模型。
...
flowchart TB
    ...
    subgraph EventCheck[contexts/eventcheck]
        CheckApp[application: evaluate · save snapshot · feedback · handoff]
        CheckDomain[domain: scope · model · evaluator · snapshot]
        CheckPorts[ports]
        CheckApp --> CheckDomain
        CheckApp --> CheckPorts
    end
...
與 restart-persistence 驗收，Event Check／Check Models 現為 canonical workspace。
...
| 能力 | 目前狀態 |
|---|---|
| Event Check／Check Models backend API、Snapshot persistence、Case handoff 與 workspace UI | canonical；EH-ECM-006 全量與 restart 驗收已完成 |
| Timeline、Journey、Journey Profile Registry、Pattern Library 舊入口 | bounded compatibility adapter；廣泛 Timeline 與未知 reference 保留唯讀，不作 canonical authoring／decision source |
| Investigation Cases、Ingestion Issues、Scenario Lab、Guide | 已實作並保留獨立責任 |
...
- Canonical Check Model authoring：[contracts/check-models](../../contracts/check-models)
- Deterministic Event Check fixtures：[contracts/event-check/fixtures](../../contracts/event-check/fixtures)
- Legacy Journey／Pattern authoring（compatibility only）：[contracts/journeys](../../contracts/journeys)／[contracts/patterns](../../contracts/patterns)
- Backend capability boundaries：[application-screaming-architecture.md](application-screaming-architecture.md)
- Current architecture PlantUML：[event-hunter-architecture.puml](diagrams/event-hunter-architecture.puml)
- Current activity diagram source：[event-hunter-activity.puml](diagrams/event-hunter-activity.puml)
